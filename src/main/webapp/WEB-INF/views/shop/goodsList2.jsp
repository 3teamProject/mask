...
<table class="GOODS_LIST" style="width:'100%'">
      ...
      <c:forEach items="${list}" var="list">
         <div class="card">
           ...
           <%--  <c:set var="IMG" value="${fn:split(list.GOODS_IMG_THUM,',')}"/>
            <div class="imgswap"> 
               <img src="/muscle/img/goods_upload/${IMG[0]}" width="400" height="400"> 
               <img src="/muscle/img/goods_upload/${IMG[1]}" width="400" height="400">
             </div>
            <c:set var="HASH" value="${fn:split(list.GOODS_HASH,',')}"/>
               <c:forEach var="HASH1" items="${HASH}" varStatus="g">
                  <c:if test="${g.count == 1 }">
                      <span style="background-color:#ff80bf; line-height: 27px; border-radius: 10px;"><font color="#ffffff" size="2" >${HASH1}</font></span>
                  </c:if>
                  <c:if test="${g.count == 2 }">
                      <span style="background-color:#d456dc; line-height: 27px; border-radius: 10px;"><font color="#ffffff" size="2">${HASH1}</font></span>
                  </c:if>
                  <c:if test="${g.count == 3 }">
                      <span style="background-color:#33b7ff; line-height: 27px; border-radius: 10px;"><font color="#ffffff" size="2">${HASH1}</font></span>
                  </c:if>
               </c:forEach> --%>
               <br>
         
         <font class="font1" >${list.GOODS_NAME}</font><br>
         <font class="font2"><fmt:formatNumber value="${list.GOODS_MPRICE}" pattern="#,###"> </fmt:formatNumber>원</font>
